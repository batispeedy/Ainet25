...
Route::middleware(['auth', 'can:manage-inventory'])->group(function () {
    ...
    Route::post('/inventory/supply', [InventoryController::class, 'createSupplyOrder'])->name('inventory.supply.create');
    Route::post('/inventory/supply/{id}/complete', [InventoryController::class, 'completeSupplyOrder'])->name('inventory.supply.complete');
    Route::post('/inventory/{id}/adjust', [InventoryController::class, 'adjustStock'])->name('inventory.adjust');
});

// Gestão de Categorias, Produtos e Portes (apenas settings)
...
Route::middleware(['auth','can:manage-orders'])->group(function () {
    ...
    Route::post('/admin/orders/{id}/complete', [EmployeeOrderController::class, 'complete'])->name('admin.orders.complete');
    Route::post('/admin/orders/{id}/cancel',   [EmployeeOrderController::class, 'cancel'])->name('admin.orders.cancel');
});
...
Route::middleware('auth')
     ->get('/stats/personal', [StatsController::class, 'personal'])
     ->name('stats.personal');
